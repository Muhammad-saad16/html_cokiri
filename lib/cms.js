...
// An unreachable CMS must not break the build or a page render: callers get
// null (empty state) and the page revalidates once the CMS is back.
async function cmsFetch(path) {
  try {
    const res = await fetch(`${CMS_URL}${path}`, { next: { revalidate: 60 }, signal: AbortSignal.timeout(15000) });
    if (!res.ok) return null;
    return await res.json();
  } catch (err) {
    console.warn(`CMS request failed (${CMS_URL}${path}): ${err.message}`);
    return null;
  }
}
